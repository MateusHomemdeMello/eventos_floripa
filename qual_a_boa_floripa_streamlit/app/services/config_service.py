...
API_FIELDS = {'OPENAI_API_KEY':'openai_api_key', 'APIFY_API_TOKEN':'apify_api_token', 'HERE_API_KEY':'here_api_key'}
...
def parse_settings(text):
    try:
        ...
        raise ValueError('JSON inválido. Confira aspas, vírgulas e chaves.') from None
    if not isinstance(data,dict): raise ValueError('A configuração deve ser um objeto JSON.')
    credentials={}
    for name in API_FIELDS:
        if name in data:
            value=data.pop(name)
            if not isinstance(value,str): raise ValueError(f'{name} deve ser um texto.')
            credentials[name]=value.strip()
    return parse_config(json.dumps(data)), credentials


def export_config(config, secrets=None):
    data=asdict(config)
    if secrets is not None:
        data.update({name:getattr(secrets,field) for name,field in API_FIELDS.items()})
